...
from .answer_normalizer import AnswerContext, DEFAULT_NORMALIZER
...
def diagnose_record(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    protocol_tokens = ("final_answer", "verdict:", "confidence:", "selected:", "<answer", "<final")
    if not prediction or not prediction_key or any(token in prediction.lower() for token in protocol_tokens):
    ...
    for candidate in _trace_entries(row, "candidate"):
        key = _key(candidate.get("normalized_answer") or candidate.get("extracted_answer", ""), row)
        if key:
            candidate_keys.append(key)
    for refined in _trace_entries(row, "refine"):
        key = _key(refined.get("refined_normalized_answer") or refined.get("refined_answer", ""), row)
        if key:
            candidate_keys.append(key)

    for lock in _trace_entries(row, "consensus_lock"):
        lock_key = _key(lock.get("answer", ""), row)
        if lock_key and lock_key != expected_key and candidate_keys.count(lock_key) >= 2:
            causes.append("wrong_consensus")
            evidence.append(
                {
                    "signal": "wrong_consensus_lock",
                    "answer": lock_key,
                    "support": candidate_keys.count(lock_key),
                }
            )

    verifier_errors = []
    for verifier in _trace_entries(row, "attack_verifier"):
        answer_key = _key(verifier.get("candidate_answer", ""), row)
        ...
        if accepted and answer_key and answer_key != expected_key:
            verifier_errors.append({"kind": "false_accept", "answer": answer_key, "role": verifier.get("role")})
        elif rejected and answer_key == expected_key:
    ...
    if judges:
        ...
        if judge_key != expected_key and expected_key in candidate_keys:
    ...
    if isinstance(subjects, str):
        subjects = [subjects]
    aliases = {"counting_and_probability": "probability", "precalculus": "algebra"}
    expected_subjects = {aliases.get(str(subject).lower(), str(subject).lower()) for subject in subjects}
...
def build_error_report(records: Iterable[Dict[str, Any]], example_limit: int = 5) -> Dict[str, Any]:
    ...
    covered = sum(count for cause, count in primary_counts.items() if cause != "solver_math_error")
    return {
        "schema_version": 1,
        "total": len(rows),
        "correct": len(rows) - wrong,
        "wrong": wrong,
        "diagnostic_coverage": round(covered / wrong, 4) if wrong else 1.0,
...
def render_error_report_markdown(report: Dict[str, Any]) -> str:
    lines = [
        ...
        f"- 正确：{report['correct']}",
        f"- 错误：{report['wrong']}",
        f"- 可定位覆盖率：{report['diagnostic_coverage']:.1%}",
        "",
        "## 主错因分布",
        "",
        "| 错因 | 数量 | 占错误比例 |",
